...
//!
//! Queries a CometBFT node's RPC endpoint to discover the genesis file
//! and current peer set, then generates local config files so that a new
...
async fn discover_peers(url: &url::Url) -> Result<String> {
    ...
    match fetch_node_info(&client, base_url).await {
        Ok(info) => {
            tracing::info!(
    ...
    match fetch_peers(&client, base_url).await {
        Ok(extra_peers) => {
...
async fn fetch_node_info(client: &reqwest::Client, base_url: &str) -> Result<PeerInfo> {
    ...
    let listen_addr = rewrite_unspecified_host(&listen_addr, base_url);
...
async fn fetch_peers(client: &reqwest::Client, base_url: &str) -> Result<Vec<PeerInfo>> {
    let url = format!("{}/net_info", base_url);
    tracing::debug!(%url, "fetching net_info");
    ...
    for raw_peer in &raw_peers {
        ...
        if let (Some(id), Some(addr)) = (node_id, listen_addr) {
            ...
            let addr = rewrite_unspecified_host(&addr, base_url);
...
fn rewrite_unspecified_host(addr: &str, base_url: &str) -> String {
    if let Ok(sa) = addr.parse::<SocketAddr>() {
        if sa.ip().is_unspecified() {
            // Extract host from the base_url.
...
fn extract_host(url: &str) -> Option<String> {
    // Simple extraction: strip scheme, then take up to the port or path.
...
mod tests {
    ...
    fn test_rewrite_unspecified_host() {
        assert_eq!(
            rewrite_unspecified_host("0.0.0.0:26656", "http://127.0.0.1:26657"),
            "127.0.0.1:26656"
        );
        assert_eq!(
            rewrite_unspecified_host("10.0.0.5:26656", "http://127.0.0.1:26657"),
            "10.0.0.5:26656"
        );
    }
    ...
    fn test_extract_host() {
        assert_eq!(
            extract_host("http://127.0.0.1:26657"),
            Some("127.0.0.1".into())
        );
        assert_eq!(
            extract_host("https://node.example.com:443/path"),
