...
/// a CometBFT RPC endpoint with --cometbft-url.
///
/// Examples:
...
pub struct JoinNetwork {
    ...
    /// May be specified multiple times. If --cometbft-url is also given,
    /// these are combined with auto-discovered peers.
    #[arg(long)]
    pub peer: Vec<String>,
        ...
        let peer_source = match (&self.cometbft_url, self.peer.is_empty()) {
            (Some(url), _) => {
                if !self.peer.is_empty() {
                    tracing::info!(
                        "explicit --peer flags will be merged with auto-discovered peers"
                    );
                }
                PeerSource::CometbftRpc(url.clone())
            }
            (None, false) => PeerSource::Explicit(self.peer),
            (None, true) => {
                return Err(color_eyre::eyre::eyre!(
                    "at least one of --peer or --cometbft-url is required for peer discovery"
                ));
            }
        };
